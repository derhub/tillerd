...
pub fn parse_spec(blob: &str) -> Result<LaunchSpec> {
    let raw: serde_json::Value =
        serde_json::from_str(blob).map_err(|e| Error::LaunchSpecInvalid(e.to_string()))?;

    match raw.get("version") {
        None | Some(serde_json::Value::Null) => {
            return Err(Error::LaunchSpecInvalid(
    ...
    let spec: LaunchSpec =
        serde_json::from_value(raw).map_err(|e| Error::LaunchSpecInvalid(e.to_string()))?;

    if spec.version == 0 {
        return Err(Error::LaunchSpecInvalid("version must be >= 1".to_string()));
    }
...
pub fn migrate(blob: &str, from_version: u32) -> Result<(LaunchSpec, Option<String>)> {
    if from_version > CURRENT_SPEC_VERSION {
        return Err(Error::LaunchSpecVersionTooNew {
            found: from_version,
        ...
        for item in &self.items {
            if let Some(placement) = &item.placement {
                if !seen.insert(placement.as_str()) {
                    return Err(Error::LaunchSpecInvalid(format!(
...
mod tests {
    ...
    fn migrate_unknown_future_version_is_refused() {
        assert!(migrate(v1_blob_empty(), 99).is_err());
    }
    ...
    fn ensure_unique_placements_rejects_duplicates() {
        ...
        let spec = parse_spec(blob).unwrap();
        assert!(spec.ensure_unique_placements().is_err());
    }
