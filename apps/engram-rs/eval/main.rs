...
//!
//! Run from the crate dir: cargo run --bin engram-eval

use engram::eval::{Averaged, Metrics};
use engram::{ChunkKind, Engram, NewChunk};
...
fn main() -> anyhow::Result<()> {
    ...
    let base = std::env::temp_dir().join(format!("engram-eval-{}", std::process::id()));
    std::fs::create_dir_all(&base)?;
    let engram = Engram::open(base.join("engram.db"))?;
    ...
    engram.embed_pending(1_000_000)?;
    ...
    for q in &queries {
        ...
        let ranked_engram = engram.rank(&q.query, now(), RETRIEVAL_DEPTH)?;
        let latency_ms = started.elapsed().as_secs_f64() * 1000.0;
        let ranked: Vec<i64> = ranked_engram
            .iter()
        ...
        by_category
            ...
            .or_default()
            .add(&ranked, &gold, latency_ms, ranked.len());
    }

    let avg = overall.averaged();
    println!(
        "\nengram retrieval eval — {} chunks, {} queries, depth {}\n",
        corpus.len(),
        queries.len(),
        RETRIEVAL_DEPTH
    );
