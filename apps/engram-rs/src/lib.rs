//! engram — a local-first memory layer: capture, hybrid recall, consolidation,
...
impl Engram {
    ...
    /// embedding is left pending for the out-of-band embedder.
    ...
    pub fn index_project(&self, cwd: impl AsRef<std::path::Path>, ts: i64) -> anyhow::Result<usize> {
...
mod tests {
    ...
    fn search_returns_full_content_in_one_call() {
        let dir = tempfile::tempdir().unwrap();
        let e = Engram::open(dir.path().join("engram.db")).unwrap();
        e.ingest(doc("the project uses postgres as the main datastore for everything"))
            .unwrap();
        e.embed_pending(100).unwrap();

        let results = e.search("postgres datastore", 10, 5).unwrap();
        assert_eq!(results.len(), 1);
        assert!(results[0].content.contains("postgres as the main datastore"));
    }
    ...
    fn opening_with_a_new_model_drops_stale_vectors_and_backfills() {
        ...
        {
            let e = Engram::open_with(
                &db,
                Box::new(embed::StubEmbedder::with_id(256, "model-a")),
            )
            .unwrap();
            e.ingest(doc("the cache uses an LRU eviction policy")).unwrap();
            e.embed_pending(100).unwrap();
            let n: i64 = e
                .store()
                .conn()
                .query_row("SELECT COUNT(*) FROM embeddings WHERE model='model-a'", [], |r| {
                    r.get(0)
                })
                .unwrap();
            assert_eq!(n, 1);
        ...
        let e = Engram::open_with(
            &db,
            Box::new(embed::StubEmbedder::with_id(256, "model-b")),
        )
        .unwrap();
        let stale: i64 = e
            .store()
    ...
    fn recall_on_empty_store_is_uncertain() {
        ...
        assert!(matches!(
            ...
            RecallResult::Uncertain { offer_archive: true }
        ));
    }
    ...
    fn evict_then_archive_recall_round_trip() {
        ...
        assert_eq!(moved, 1);
        assert_eq!(e.active_chunk_count().unwrap(), 0, "chunk left the active set");
        ...
        assert!(!arch.is_empty(), "archive search recovers the evicted chunk");
        assert!(arch[0].snippet.contains("redis"));
    }

    /// A store with one session, one project, and a chunk at ts=100.
    fn engram_with_recent_chunk() -> (tempfile::TempDir, Engram) {
        let dir = tempfile::tempdir().unwrap();
        let e = Engram::open(dir.path().join("engram.db")).unwrap();
        e.store()
            .conn()
    ...
    fn context_withholds_a_digest_older_than_the_latest_chunk() {
        let (_d, e) = engram_with_recent_chunk();
        e.store()
