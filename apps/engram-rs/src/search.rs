...
//! at engram's scale) fused with FTS5 lexical search.
...
pub fn rrf_fuse_weighted(
    ...
    let mut fused: Vec<Scored> = acc.into_iter().map(|(id, score)| Scored { id, score }).collect();
...
pub fn is_symbol_like(query: &str) -> bool {
    ...
    let has_camel = q.chars().zip(q.chars().skip(1)).any(|(a, b)| {
...
mod tests {
    ...
    fn rrf_prefers_items_ranked_by_both() {
        ...
        assert_eq!(fused[0].id, 2, "item present in both lists should rank first");
    }
