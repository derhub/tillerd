...
fn split_large(sec: &DocChunk, out: &mut Vec<DocChunk>) {
    ...
    for (i, line) in lines.iter().enumerate() {
        ...
        if at_paragraph_break && big_enough && more_coming {
            ...
            buf = buf[start..].iter().copied().filter(|l| !is_fence(l)).collect();
...
pub fn index_project(engram: &Engram, cwd: &Path, ts: i64) -> anyhow::Result<usize> {
    let cwd_str = cwd.to_string_lossy().to_string();
    engram.store().delete_doc_chunks_under(&cwd_str)?;
...
mod tests {
    ...
    fn a_hash_inside_a_code_fence_is_not_a_heading() {
        ...
        assert_eq!(chunks.len(), 1, "the fenced '# ...' must not start a new chunk");
    ...
    fn fenced_block_in_oversized_section_is_never_split() {
        ...
        for c in chunk_markdown(&md) {
            ...
            assert!(fences == 0 || fences == 2, "a chunk must not contain a half-open fence");
        }
    }
    ...
    fn excludes_heavy_directories() {
        ...
        std::fs::write(proj.join("node_modules/skip.md"), "## Skip\nshould not index").unwrap();
    ...
    fn indexes_documents_verbatim() {
        ...
        let content: String = e
            .store()
            .conn()
            .query_row("SELECT content FROM chunks WHERE kind='doc'", [], |r| r.get(0))
            .unwrap();
        assert!(content.contains("middle after"), "doc kept verbatim, not truncated");
    }
    ...
    fn splits_at_sentence_boundaries() {
        ...
        assert!(parts.iter().all(|p| p.chars().count() <= 35), "all parts fit within limit");
    }
    ...
    fn sentence_split_falls_back_to_char_boundary() {
        ...
        assert_eq!(parts[0].chars().count(), 1500, "first chunk is exactly max size");
    }
