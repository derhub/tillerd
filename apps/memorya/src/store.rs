...
/// How long a writer waits on a locked database before erroring. The store is
/// shared behind a mutex, so contention is rare; this is belt-and-suspenders for
/// the WAL checkpoint window.
const BUSY_TIMEOUT: Duration = Duration::from_millis(5_000);

/// Hex SHA-256 of a chunk's content, used for content-hash dedup.
...
impl Store {
    ...
    pub fn insert_chunk(&self, c: &NewChunk) -> anyhow::Result<Option<i64>> {
        let n = self.conn.execute(
            "INSERT OR IGNORE INTO chunks
               (session_id, kind, content, title, file_path, turn_index, ts, content_hash)
    ...
    /// database.
    pub fn move_chunks_to_archive(&self, archive_path: &str, ids: &[i64]) -> anyhow::Result<usize> {
        if ids.is_empty() {
            return Ok(0);
        }
        self.conn
...
mod tests {
    ...
    fn prune_docs_removes_only_document_chunks() {
        ...
        s.conn()
            .execute(
                "INSERT INTO sessions(id, ide, started_at) VALUES ('s1','t',0)",
                [],
            )
            .unwrap();
        s.insert_chunk(&chunk("s1", 0, "conversation turn"))
            .unwrap();
