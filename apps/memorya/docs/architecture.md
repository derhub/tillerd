...
```rust
memorya.ingest(chunk)                  // raw chunk
memorya.capture_prompt(...) / capture_tool(...)
memorya.index_project(cwd, ts)         // index project markdown
memorya.recall(query, now) -> RecallResult
memorya.search(query, now, k) -> Vec<SearchResult>   // one-shot, full content
memorya.rank(query, now, k) -> Vec<i64>              // ranked ids (eval)
memorya.expand(id) / expand_many(ids)
memorya.entity(name) / learn(...) / forget(...)
memorya.consolidate(...) / evict(...) / prune_docs() / prune_all()
```
