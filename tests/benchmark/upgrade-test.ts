...
function manifestPid(): number | null {
  try {
    return JSON.parse(readFileSync(manifestPath, "utf8")).pid;
  } catch {
    return null;
  }
}
...
await c1.connect();
c1.on((f) => {
  if (f.meta?.type === "data" && f.meta.sessionId === "surv" && f.body)
    c1.send({ type: "ack", sessionId: "surv", bytes: f.body.length });
});
c1.send({
  type: "spawn",
  sessionId: "surv",
...
c2.on((f) => {
  if (f.meta?.type === "data" && f.meta.sessionId === "surv" && f.body)
    c2.send({ type: "ack", sessionId: "surv", bytes: f.body.length });
});
