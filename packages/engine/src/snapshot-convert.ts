import type { SnapshotFrame } from "@athing/sdk";
...
export function snapshotToBytes(frame: Pick<SnapshotFrame, "rows" | "cols" | "cells" | "cursor">): Uint8Array {
  const parts: string[] = [];
  ...
  for (let row = 0; row < frame.rows; row++) {
    for (let col = 0; col < frame.cols; ) {
      const cell = frame.cells[row]?.[col];
      if (!cell) { col++; continue; }

      // Skip wide-char continuation cells
      if (cell.char === "") { col++; continue; }

      // Skip default empty spaces
      if (cell.char === " " && cell.fg === COLOR_DEFAULT && cell.bg === COLOR_DEFAULT && cell.attrs === 0) {
        col++;
        continue;
      }
      ...
      const cp = cell.char.codePointAt(0) ?? 0x20;
