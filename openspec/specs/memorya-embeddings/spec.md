...
Defines how the memory layer turns chunks and queries into semantically meaningful vectors using a static, CPU-only embedding model behind a stable embedder interface: model identity and dimensionality tagging, first-run download with offline reuse, and model-safe recall with out-of-band backfill.
