# ui-progressive-boot Specification

## Purpose

Defines progressive boot: the application shell renders before services are ready, service-dependent content lazy-loads behind a delayed skeleton (so fast resolves never flash a placeholder), and a service failure during boot degrades to the read-only health indicator rather than a blocking screen.
