...
The launch executor drives a session's launch spec to life: it iterates items in order, resolves commands, dispatches surfaces to the correct adapter by kind, runs optional worktree steps, persists placement, and copies the spec atomically from templates.
