...
The native desktop application that hosts the shared renderer in an embedded system web view and
drives the application lifecycle, gracefully shutting down the background processes it owns on
exit.
...
(label + intent query) and to raise an existing window to the front by its label. All windows SHALL
share the single embedded orchestrator backend; opening a child window SHALL NOT spawn a backend. A
window closes itself through the core window API; closing one SHALL NOT tear down the backend.
