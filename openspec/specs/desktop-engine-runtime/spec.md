...
Running the agent engine and adapter inside the desktop web view over injected ports, constructed
from the values the native core resolves at startup, with the renderer selecting the native
transport on desktop and the network transport on web.
