...
Defines the read-only per-service health capability: the orchestrator derives each supervised service's state from its existing discovery record (the manifest, the single discovery source) and delivers it to the interface through a host-agnostic source, adding no health probe, socket, or route to any service.
