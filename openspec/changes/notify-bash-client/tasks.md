...
- [ ] 1.1 `bin/` is currently fully gitignored (generated wrappers only). Un-ignore the source
      client: add `!athing-notify` to `bin/.gitignore`.
- [ ] 1.2 Add `bin/athing-notify` (`#!/usr/bin/env bash`, no extension), committed as source.
...
- [ ] 1.3 Support both bridge forms (mirroring current `notify.ts`): a value beginning with `/`
      delivers over the local control-channel socket (no network port); any other value is a URL.
- [ ] 1.4 Fire-and-forget: bound runtime (short max time), discard output, suppress all errors,
      always `exit 0`. If `$ATHING_BRIDGE_URL` is unset/empty, exit 0 immediately.
- [ ] 1.5 `chmod +x bin/athing-notify`; confirm committed executable.

## 2. Host resolves and points the hook command at the shell client

- [ ] 2.1 Rewrite `notifyScriptPath()` in `packages/platform-bun/src/ingress.ts` to resolve the
...
- [ ] 2.2 Change `notifyCommand()` to return that absolute path directly (drop the `bun ` prefix).
- [ ] 2.3 Update `prepareNotifyScript()` to verify the resolved path exists and is executable;
      keep the typed `HookInstallFailed` error when it is absent (update the message — no longer
      "run: bun run build").

## 3. Remove the runtime-based client

- [ ] 3.1 Delete `packages/platform-bun/src/notify.ts`.
- [ ] 3.2 Remove the `build` script in `packages/platform-bun/package.json` that emits
      `notify.mjs` into `$ATHING_DIR` (no build step remains for the hook client).
- [ ] 3.3 Update the adapter `HOOK_MARKER` in `hook-installer.ts` from `"notify.mjs"` to
      `"athing-notify"` so install/uninstall idempotency still detects the entry.

## 4. Tests

- [ ] 4.1 `bun test` in platform-bun: start a throwaway local unix-socket receiver, exec
...
- [ ] 4.2 No-bridge case: unset `ATHING_BRIDGE_URL`, exec the client, expect exit 0 and no delivery.
- [ ] 4.3 Fire-and-forget: point the bridge at a dead/slow endpoint, exec the client, expect exit 0
      within the bound and no thrown error.
- [ ] 4.4 Update/extend `packages/platform-bun/tests/supervisor.test.ts` (or a new
...
- [ ] 4.5 Run `bun test` across the workspace + the daemon hook-ingress tests to confirm the
      end-to-end hook path (agent -> client -> ingress -> relay) stays green.

## 5. Docs

- [ ] 5.1 Note the runtime-free shell client and the `curl`-on-PATH assumption (macOS/Linux v1) in
