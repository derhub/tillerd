...
- **Known limitation** → Ctrl+C pressed *inside* the agent terminal delivers SIGINT via forwarded input bytes, not an engine frame; if it exits the agent it classifies `unexpected` for v1.
