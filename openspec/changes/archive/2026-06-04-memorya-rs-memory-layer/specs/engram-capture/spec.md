...
### Requirement: Private-content redaction

The memory layer SHALL strip content marked private before any write, so private content is never persisted.

#### Scenario: Private spans removed before storage

- **WHEN** captured content contains a span marked private
- **THEN** that span MUST be removed before the content is written
- **AND** the private span MUST NOT appear in any stored chunk, digest, or fact
