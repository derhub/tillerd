...
- [x] 1.1 Create `apps/engram-rs` crate with `Cargo.toml` and the module layout (`redact`, `embed`, `tool_use`, `indexer`, `entity`, `fact`, `search`, `coverage`, `jobs`, `archive`, `mcp`, `server`, `store`, `lib`); curation modules out of scope; `inject` folded into `lib::context`; `jobs`/`archive` are single files
...
## 2. Storage layer (engram-storage)
...
## 3. Embedding (engram-storage / engram-capture)
...
## 6. Consolidation & eviction (engram-consolidation)
...
## 7. Archive (engram-consolidation)
