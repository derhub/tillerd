...
- Wire `@athing/memorya` capture to the crate: `capture_prompt` and `capture_tool` redact
...
- `memorya-capture`: re-introduce capture-time redaction — prompts and tool events (response
...
- Code: new `packages/redact-rs` (`Cargo.toml`, `src/lib.rs`, `src/patterns.rs`,
  `src/entropy.rs`, `src/main.rs`); workspace membership; `apps/memorya-rs/Cargo.toml`
  (depend on `redact-rs`) and `apps/memorya-rs/src/lib.rs` (call redaction in
