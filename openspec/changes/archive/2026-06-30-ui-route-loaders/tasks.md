...
- [x] 4.2 Full e2e green (boot/pending, nav, detach/re-attach, workspace/project scoping, logs `?service`, reload-deep-route) + Rust + ast-grep + `bun run verify`.
- [x] 4.3 Docs `docs/tanstack-client-engine.md` + memory `client-engine.md`: render-as-you-fetch, non-awaited loaders, queryFn readiness (`withClient`), frontend composition, realtime = streams patch cache.
...
- [x] 5.6 Async-ban compliant (sg scan clean). Unit + `bun run verify` green (UI 246). E2E specs updated and desktop E2E verified fully green (all 37 scenario tests pass).
