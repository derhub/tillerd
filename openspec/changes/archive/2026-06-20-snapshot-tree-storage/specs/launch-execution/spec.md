...
**Reason**: The worktree entity and `git worktree add` provisioning step are dropped in 0.0.15 slice 1 (ADR-0033). A working directory is now expressed as `cwd` set directly on the launch item / surface; no separate worktree provisioning step exists.

**Migration**: Remove any worktree step from launch items. Set `cwd` directly on the launch item or surface to specify its working directory relative to the project root.
