...
- [x] 3.4 Adapt the `Admin` handler to the post-preamble stream: request/response loop unchanged; admin-token credential now enforced by the policy in .3; drop its own listener bind
