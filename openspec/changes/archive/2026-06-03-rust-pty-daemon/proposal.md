...
- Add a benchmark harness under `tests/benchmark` that drives both daemons through identical
...
- `daemon-benchmark-harness`: A reproducible benchmark suite under `tests/benchmark` that runs both
...
- **New code**: a Rust crate (workspace-external to the Bun/turbo graph) producing an
  `athing-daemon`-compatible binary; a `tests/benchmark` harness (Bun-driven runner that speaks the
