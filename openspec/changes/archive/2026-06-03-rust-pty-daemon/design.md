...
ported **1:1 to Rust** (`packages/daemon-rs/src/vt.rs`) rather than using `alacritty_terminal`. The spec's
hard requirement is snapshot *parity with the reference daemon*; an off-the-shelf VT engine
...
- _Parity evidence_: `vt::tests::snapshot_parity_with_reference` feeds a synthetic ANSI stream
  ...
  the reference `VtState` (`packages/daemon-rs/tests/fixtures/vt-golden.json`).
...
The Rust daemon is a standalone cargo crate at `packages/daemon-rs/`, built via `cargo build
...
Task 11.6 ("run against both the reference and Rust daemons") requires a *working* reference. The
