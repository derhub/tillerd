...
- [x] 2.2 Extract `fs.rs` plumbing to sibling helpers (`slug.rs`, `atomic_io.rs`, `index.rs`,
...
- [x] 3.1 `store/backend.rs`: closed `enum Backend { Fs(FsBackend), Sqlite(SqliteBackend),
...
- [x] 3.2 One per-entity async store struct per entity -- domain (`Workspaces`/`Projects`/`Sessions`/
...
- [x] 4.1 Delete `DomainStore`/`OperationalStore`/`Store`/`CompositeStore`, the `persistence/`
...
- [x] 4.2 Add the standalone `create_session(draft, &LaunchTemplates, &Sessions)` coordinator fn at
...
- [x] 4.3 Update consumers to take only the concrete stores they use (least-privilege), not the whole
