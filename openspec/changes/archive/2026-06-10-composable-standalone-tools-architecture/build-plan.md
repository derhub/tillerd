...
- Dual-mode detection mechanism for memory (6.7): how the tool distinguishes 'memorya serve' (viewer only) vs 'memorya mcp' (standalone stdio) vs orchestrator-launched composed (env var vs flag vs no-args). Ambiguous detection runs the wrong startup path; pin the contract before recall dual-mode tests.
...
- Existing touchpoints:
  ...
  - apps/memorya-rs/src/main.rs: POST /hook handler removal (task 6.9)
  - apps/memorya-rs/src/lib.rs: add HookSource port with gate-subscription adapter (task 6.3), add durable capture-queue table and worker (task 6.5-6.6)
...
### memorya-capture — `apps/memorya-rs`
...
- New modules:
  - `apps/memorya-rs/src/hook_source.rs — HookSource port trait + gate-subscription adapter + stub source for tests`
  - `apps/memorya-rs/src/capture_queue.rs — durable queue table schema + operations (insert, drain, reclaim stale)`
  - `apps/memorya-rs/src/worker.rs — proactive background worker (spawn on init, drain on blocking pool, restart on panic)`
- Existing touchpoints:
  - apps/memorya-rs/src/lib.rs:114-119 — Engram struct gains hook_source: Arc<dyn HookSource> field; constructor passes it
  - apps/memorya-rs/src/lib.rs:181-183 — ingest() remains synchronous, but enqueues embedding async (returns immediately)
  - apps/memorya-rs/src/lib.rs:321-323 — embed_pending() becomes internal; replace with worker drain (no public embed call on hot path)
  - apps/memorya-rs/src/store.rs:42-61 — insert_chunk() signature unchanged, idempotent via UNIQUE constraint (already exists)
  - apps/memorya-rs/src/schema.sql — add capture_queue table: (id, chunk_id, status, attempts, last_tried, created_at)
  - apps/memorya-rs/src/server.rs:1-2 — remove POST /hook handler (ingest_hook function deleted; line 38-47, 57-112)
  - apps/memorya-rs/src/server.rs:35-50 — route() function: remove ('POST', '/hook') case; keep ('GET', '/') viewer + health
  - apps/memorya-rs/src/main.rs:155-158 — 'serve' command kept for human viewer only (GET / + /healthz); no listen for hooks
  - apps/memorya-rs/Cargo.toml — add: tokio (async runtime), contracts-rs (HookEvent type), gate-client (subscription decode)

### memorya-recall — `apps/memorya-rs`

- Depends on: contracts-rs (wire types HookEvent, SessionId, CorrelationId per task 1.1); service-host (memorya binary lifecycle, task 2.1-2.4); gate-client (HookSource gate-subscription adapter, task 6.2); gate (operational before memory can subscribe, task 5.x); memorya-capture (task 6.3-6.4 depend on HookSource port being defined here)
- New modules:
  - `apps/memorya-rs/src/dual_mode.rs: Dual-mode configuration/initialization (standalone vs composed detection)`
  - `apps/memorya-rs/src/mcp_server.rs: Split from existing mcp.rs to separate stdio MCP server logic (standalone-only)`
  - `apps/memorya-rs/src/viewer_server.rs: Split from existing server.rs to retain GET / viewer endpoint (both modes)`
  - `Cargo.toml additions: service-host dependency, gate-client dependency`
- Existing touchpoints:
  - apps/memorya-rs/src/server.rs:34-50: route() — DELETE POST /hook branch (ingest_hook removal), RETAIN GET / and GET /healthz
  - apps/memorya-rs/src/server.rs:59-112: ingest_hook() — DELETE entirely (capture now via HookSource gate subscription per 6.3-6.4)
  - apps/memorya-rs/src/server.rs:139-188: serve() — RETAIN for viewer only, rename to serve_viewer()
  - apps/memorya-rs/src/main.rs:152-158: 'serve' command — UPDATE to call renamed serve_viewer(), add dual-mode startup
  - apps/memorya-rs/src/main.rs:152-154: 'mcp' command — RETAIN for standalone-only mode
  - apps/memorya-rs/src/lib.rs:114-152: Engram struct and open() — ADD optional HookSource port dependency (6.3)
  - apps/memorya-rs/src/lib.rs:1-17: Module declarations — ADD new dual_mode, mcp_server, viewer_server modules

### tool-composition — `cross-cutting: apps/mcp-gateway-rs + orchestrators (server/desktop/cli)`

- Depends on: service-host (task 2.1-2.4); process-launch (task 2.5-2.6); contracts-rs (task 1.1-1.2); gate (tasks 5.1-5.14); daemon-session-subscription (task 3.1-3.5); memorya-capture (task 6.1-6.4 HookSource port); rust-pty-daemon becomes pty-only (task 3.1-3.4)
- Existing touchpoints:
  ...
  - apps/memorya-rs/Cargo.toml (memory adopts service-host, adds gate-client dependency)
