...
## 6. Wire engram capture

- [x] 6.1 `apps/engram-rs/Cargo.toml`: depend on `redact-rs`
...
- [x] 6.4 engram test: prompt + tool capture store `[REDACTED]`; document indexing stays verbatim

## 7. Verify

- [x] 7.1 `cargo clippy --all-targets -- -D warnings` clean; `cargo test` green for both crates
- [x] 7.2 Runtime: drive `engram serve` → `POST /hook` with a secret → stored chunk shows `[REDACTED]`
