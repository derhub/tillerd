...
`@athing/engram` persists captured prompts and tool events as chunks (SQLite + embeddings)
...
`packages/` (`daemon-rs`, `platform-rs`); `regex` is the de-facto pure-Rust engine and is
...
- Wire engram capture (prompt, tool body, tool-input title) before `ingest`.
...
A workspace library crate (`engram` depends on it), plus a `redact` binary (`src/main.rs`)
...
Single linear pass. Thresholds are constants, tuned against the engram eval corpus.
...
### D7: engram wiring
`apps/engram-rs/Cargo.toml` depends on `redact-rs`. In `lib.rs`, `capture_prompt` wraps
...
(prune + re-index for a clean store). Rollback: drop the engram dependency call; stored data
