...
orchestrator/src/
  ...
  store/      per-entity async stores (Workspaces/Projects/Sessions/Surfaces) + the closed `Backend` enum
  app/        use cases  (R2)
```
...
- **`store/` -- per-entity async stores + the `Backend` enum.** A closed `enum Backend { Fs(..),
  Sqlite(..), Memory(..) }` wraps the `infra` backends. One concrete async store struct per entity
  (`Workspaces`, `Projects`, `Sessions`, `Surfaces`) holds a `Backend` and exposes that entity's
  operations -- `get / list(&Filter) / create / update / delete` plus entity-specifics (reorder,
  archive-representation) -- dispatching by `match`. `Filter` is a declarative, typed per-entity
  struct (`#[derive(Default)] ProjectFilter { workspace: Option<WorkspaceId>, include_archived: bool }`)
  the backend pushes down. This **dissolves `DomainStore` / `OperationalStore` / `CompositeStore`**;
  which `Backend` serves which entity is a composition-root choice.
- **Coordination forced out by dissolving the facade** (`create_session` template resolution; the
  archive decision) lands minimally at the **composition root** in R1a, formalized into `app/` use
  cases in R2 -- behavior-identical here.
...
- **Code (`crates/orchestrator/src/`)**: new `entities/` (types out of `persistence/mod.rs`); new
  ...
  root rewired to assign entity -> `Backend` and host the forced-out coordination. Call sites adapt to
  the async store API (the crate is already async). No change to surface/launch.
