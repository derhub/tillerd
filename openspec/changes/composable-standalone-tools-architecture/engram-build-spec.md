...
Synthesized from the `engram-build-spec` design workflow (6 agents, 2026-06-07), grounded in the real `engram-rs` + `contracts-rs` code. TDD-first, inline. One commit per `tasks.md` item. Depends on `contracts-rs` (done) and the gate's subscribe wire (5.9 — reconcile gate-client fixtures when the gate build lands).

## Resolved decisions (accepted from design openDecisions)

- **Sync, no async.** engram stays fully synchronous: `std::thread` + blocking `std::os::unix::net::UnixStream` + a transport-free codec. Do **not** add tokio/async-trait (consistent with the existing zero-async engram + transport-free `daemon-pty-client`).
...
- NEW `packages/gate-client`: `contracts-rs` (as `contracts`), `serde` (derive), `serde_json`, `thiserror = "2"`. NO tokio, NO daemon-pty-client, NO engram. Add to root workspace members. package.json (lib turbo), `.gitignore /target`, no per-crate lock/profile.
- `apps/engram-rs` add: `gate-client = { path = "../../packages/gate-client" }`, `contracts-rs` (as `contracts`), `sha2 = "0.10"`. Already has serde_json/anyhow/rusqlite/redact. NO tokio.
...
5. **Daemon-free + memory-only slice** [6.1, 6.8] — dependency-direction guard tests (engram lib has no daemon/pty/socket dep; gate-client has no engram dep; rule 8.4); memory-only slice integration test GATED on gate 5.9 (ignored until live; tracked with 8.2).
...
deps (3): engram_lib_has_no_daemon_pty_or_socket_dependency; gate_client_has_no_engram_or_daemon_pty_client_dependency; engram_wire_dependencies_are_only_contracts_rs_and_gate_client.
...
- Dep direction (8.4): gate-client reimplements framing locally, imports neither daemon-pty-client nor engram; engram imports gate-client, never the reverse — guard with a cargo-metadata test.
- Semantics flip: `engram serve` becomes viewer-only; `engram mcp` additionally starts the viewer — document + test both.
