...
dep-direction + deploy (9): no_tool_imports_another_tool_internal_modules_cargo_dag; no_tool_depends_on_orchestrator_cargo_dag; only_daemon_pty_client_knows_pty_wire; ts_gate_client_carries_zero_engram_dependency; gateway_only_deployment_boots_and_serves; full_deployment_boots_and_serves_end_to_end; correlation_id_traces_action_across_daemon_gate_gateway_memory.
...
- Dep direction (8.4): no tool→tool / tool→orchestrator; only daemon-pty-client knows the PTY wire; TS gate-client zero engram dep — enforce via Cargo DAG + TS import boundaries.
