...
- **One** raw parser → zero raw-format drift. engram's stray vocabulary becomes a trivial map from
...
| PermissionRequest | WAITING_INPUT   | skip (low-value per `engram-capture` skip-list)              |
...
- **R5 — Memory dual-mode detection.** Face by subcommand: `engram mcp` = standalone MCP stdio + viewer; `engram serve` = viewer only. Capture source by env: `ATHING_GATE_URL` present -> subscribe to the gate (composed); absent -> standalone (stub/none). (task 6.7)
