...
adopt-or-spawn, child supervision. The result is "supervisor everywhere, daemon everywhere":
...
- **Memory recall is dual-mode.** Standalone, memory exposes recall as its own standard MCP front.
  ...
  loopback (required by the `engram-recall` spec) is retained in both modes.
- **Capture queue is made durable and proactively drained.** The `engram-capture` spec already
...
- **Memory stops being an "app" with its own process-management surface. BREAKING.** Storage,
  recall, embeddings, chunking, consolidation stay in `engram-rs`'s existing library (it is already
  lib + bin); the memory tool is the thin bin over it, and ad-hoc process glue moves to
  `service-host`. No new `engram-core` crate. The loopback human viewer is retained (required by
  `engram-recall`); only the hook-ingress path and process plumbing change.
...
- New libraries: `service-host`, `process-launch`, `daemon-pty-client` (Rust, PTY wire),
  ...
  one versioned spec. (No `gate-core`/`engram-core` crates — see below.)
...
- Memory: `HookSource` port (gate-subscription adapter; stub source for tests); durable capture queue
  - worker; dual-mode recall; the memory tool is the thin bin over `engram-rs`'s existing library.
...
- `rust-pty-daemon`: becomes pty-only. The "hook ingress capability" requirement is removed (moves
  ...
  knowledge.
- `engram-capture`: capture is driven through a `HookSource` port (the gate subscription); the
  enqueue becomes durable and is drained by a proactive worker.
- `engram-recall`: recall is dual-mode — a standalone standard MCP front, or fronted by the tool
...
- **Crates:** new `service-host`, `process-launch`, `daemon-pty-client`, `gate-client`,
  ...
  `daemon-pty-rs` loses hook ingress; `engram-rs` keeps its existing library and becomes a thin memory
  tool over it (no `engram-core` crate), consuming canonical `HookEvent`s;
...
5. In `engram-rs`'s library, introduce the `HookSource` port (gate-subscription adapter; stub source
