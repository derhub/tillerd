...
A `ContextStore` holds named boolean/string keys (e.g. `terminalFocus`, `hasActiveSession`, `isDesktopHost`, `commandPaletteOpen`). Seeded from existing signals — the `isCaptureTarget`/xterm-focus guard becomes a real `terminalFocus` key; `isDesktopHost()` becomes a key. `WhenExpr` is a minimal form: a list of terms ANDed together, each term a key name or a negated key (`!terminalFocus`). Evaluator is ~20 lines, no parser dependency. `when` gates three things: palette visibility (hidden when false), keybinding activation (dispatch skips a binding whose command's `when` is false), and command enablement (disabled/greyed in menus). Alternative rejected: full grammar — more code and test surface than the scope needs; the format is forward-compatible (a richer expr type can replace the term-list later without changing call sites).

### D3: Surface tags drive the palette and the title bar

`Surface = "palette" | "titlebar" | "contextmenu"`. The palette renders defs whose `surfaces` includes `palette` (default) and whose `when` passes, grouped by `category`/`group`. The title bar toolbar renders defs tagged `titlebar` in `group` order, as icon buttons; a toggle def renders active/inactive from its `checked`. This makes the title bar toolbar a pure projection of the command table — no hand-wired buttons. Alternative rejected: full menus map — unnecessary indirection for three surfaces.
